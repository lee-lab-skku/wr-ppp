...
working directory. The `report-build` implementation lives in
`scripts/report-build`; `scripts/setup.sh` installs a link to that canonical
script rather than generating another copy.

## Validation

Run checks that are available and proportionate to the change. For shell-only
changes, syntax checks and `./scripts/test.sh` on the contributor's current
platform are useful when the required tools and configured Docker image are
available.
